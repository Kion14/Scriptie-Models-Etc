...
# 2) otherwise fall back to $TMPDIR/DAPI
# 3) otherwise use ./DAPI
DATA_DIR = os.environ.get(
    "DATA_DIR",
    os.path.join(os.environ.get("TMPDIR", "."), "DAPI")
)

# Split files
TRAIN_SPLIT_FILE = "train_indices_DAPI.pt"
VAL_SPLIT_FILE = "val_indices_DAPI.pt"
TEST_SPLIT_FILE = "test_indices_DAPI.pt"
...
class CellBinDBDataset(Dataset):
    def __init__(self, root_dir):
        ...
        if len(self.samples) == 0:
            raise RuntimeError(f"No sample folders found in dataset directory: {root_dir}")
    ...
    def __getitem__(self, idx):
        ...
        mask = np.array(Image.open(mask_path)).astype("float32")
...
def main():
    ...
    # Save predictions
    save_predictions(model, test_loader, num_samples=5)
